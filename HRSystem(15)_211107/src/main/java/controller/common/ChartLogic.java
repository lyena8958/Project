	...
	public JSONArray memYearDatas(List<MemberVO> mdatas, int dCnt){
		...
		for(int y = nowyear-dCnt+1; y <= nowyear; y++) {
			...
			for(int i = 0; i < mdatas.size(); i++) {
				...
				totalmem = totalmem += (in-out); // 연별 재직자 합산
			}
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// [최종학력]
	public JSONArray schYearDatas(List<SchoolInfoVO> sdatas, int dCnt){
		datas.clear();
		System.out.println("datas : "+datas);
		int s1; // 고졸/전문대
		int s2; // 학사
		...
		for(int y = nowyear-dCnt+1; y <= nowyear; y++) {
			...
			for(int i = 0; i < sdatas.size(); i++) {
				...
				schoolYear = Integer.parseInt(sdatas.get(i).getEndDate().substring(0,4));

				if(y != schoolYear) { // 년도가 같지 않다면 PASS
					continue;
				}
				else if(data.getFinish().equals("N")) { // 최종학력이 아니라면 PASS
					continue;
				}
	...
	public JSONArray carYearDatas(List<CareerInfoVO> cdatas, List<MemberVO> mdatas, int dCnt){
		...
		for(int y = nowyear-dCnt+1; y <= nowyear; y++) {
			...
			for(int m = 0; m < mdatas.size(); m++) {
				...
				memOutYear = Integer.parseInt(mdatas.get(m).getEndDate().substring(0,4));

				int bemnum = m==0 ?  0 : mdatas.get(m-1).getMnum(); // 이전 사원에 대한 사번

				// 입사년도와 index 같지않거나, 퇴직년도가 index가 같거나 		 (당해년도 입사와 동시에 && 퇴사한 인원은 집계제외)
				// 		  이전 사원과 사번이 같다면 PASS 				
				if(y != memInYear || y == memOutYear || mdatas.get(m).getMnum()==bemnum) {
					continue;
				}
				...
				for(int i = 0; i < cdatas.size(); i++) {
					if(!cdatas.get(i).getStartDate().isEmpty()) { // 데이터가 비어있지 않다면 == 경력대상 처리
						flag = true;
						break;
					}
				}
	...
	public JSONArray ageYearDatas(List<MemberVO> mdatas, int dCnt){
		...
		for(int y = nowyear-dCnt+1; y <= nowyear; y++) {
			...
			for(int i = 0; i < mdatas.size(); i++) {
				...
				// 퇴사일
				memYear = Integer.parseInt(mdatas.get(i).getEndDate().substring(0,4));
